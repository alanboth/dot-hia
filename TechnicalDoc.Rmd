...
The potential impact fraction (PIF) measures the proportional change in the burden of disease attributable to a change in a related risk factor[@RN19]. We used the PIFs to calculate the average change in the incidence of diseases (Figure 2) in the scenarios' diseases' processes.  We calculated the PIFs for: physical activity and air pollution. PIF inputs are baseline and scenario exposure to risk factors and dose-response functions for the association of exposure and disease combinations (e.g. physical inactivity and type 2 diabetes). We calculated the “distribution shift” PIFs given the continuous distribution of the modelled risk factors (Formula 1).  Some diseases shared risk factors, for example, air pollution and physical inactivity are risk factors for ischemic heart disease. To account for shared risk factors we used Formula 2. Physical activity levels were modelled at the individual level for the Vicotria model and at the population level for the AUstralian model. The Victoria model includes air pollution and road injuries, both exposures modelled tat the population level. 


$$
PIF_i\text {=}\frac{\int_a^b RR_{i,x}\text P_{(i,x)}\text dix \text - \int_a^b RR_{i,x}\text P^*_{(i,x)} \text dix}{\int_a^b RR_{i,x} \text P_{(i,x)} \text dix}
$$
