...
calculateProportionsSRL <- function(srl_df_location) {
  srl_df <- read.csv(srl_df_location,as.is=T, fileEncoding="UTF-8-BOM")
  
  # calculate the proportion of car, pt.drive, and pt.walk for the base and srl scenario.
  # also determine the rank of each proportion (1 for largest, 3 for smallest)
  srl_proportions <- srl_df %>%
    mutate(base_prop_car=base_car_person_trips/(base_car_person_trips+base_pt_drive_total_demand+base_pt_walk_total_demand),
           scen_prop_car=srl_car_person_trips/(srl_car_person_trips+srl_pt_drive_total_demand+srl_pt_walk_total_demand),
           base_prop_pt.walk=base_pt_walk_total_demand/(base_car_person_trips+base_pt_drive_total_demand+base_pt_walk_total_demand),
           scen_prop_pt.walk=srl_pt_walk_total_demand/(srl_car_person_trips+srl_pt_drive_total_demand+srl_pt_walk_total_demand),
           base_prop_pt.drive=base_pt_drive_total_demand/(base_car_person_trips+base_pt_drive_total_demand+base_pt_walk_total_demand),
           scen_prop_pt.drive=srl_pt_drive_total_demand/(srl_car_person_trips+srl_pt_drive_total_demand+srl_pt_walk_total_demand)
    ) %>%
    dplyr::select(sa2,base_prop_car,scen_prop_car,base_prop_pt.walk,scen_prop_pt.walk,base_prop_pt.drive,scen_prop_pt.drive) %>%
    pivot_longer(cols=base_prop_car:scen_prop_pt.drive,
                 names_to = c("scenario", "prop", "mode"),
                 names_sep = "_") %>%
    dplyr::select(-prop) %>%
    rename(prop=value) %>%
    ...
    as.data.frame()
  return(srl_proportions)
}

calculateTimeDistanceSRL <- function(srl_df_location) {
  srl_df <- read.csv(srl_df_location,as.is=T, fileEncoding="UTF-8-BOM")
  
  # Table format: sa2, mode, base_distance, base_time, scen_distance, scen_time
  # Mode is either car, pt.drive (driving to PT), or pt.walk (walking to PT)
  srl_time_and_distance <- srl_df %>%
    dplyr::select(sa2,
                  base_car_distance=base_car_distance_mean,
                  base_car_time=base_car_time_mean,
                  base_pt.drive_distance=base_pt_drive_distance_mean,
                  base_pt.drive_time=base_pt_drive_time_mean,
                  base_pt.walk_distance=base_pt_walk_distance_mean,
                  base_pt.walk_time=base_pt_walk_time_mean,
                  scen_car_distance=srl_car_distance_mean,
                  scen_car_time=srl_car_time_mean,
                  scen_pt.drive_distance=srl_pt_drive_distance_mean,
                  scen_pt.drive_time=srl_pt_drive_time_mean,
                  scen_pt.walk_distance=srl_pt_walk_distance_mean,
                  scen_pt.walk_time=srl_pt_walk_time_mean
    ) %>%
    pivot_longer(cols=base_car_distance:scen_pt.walk_time,
                 names_to = c("scenario", "mode", "measure"),
                 names_sep = "_") %>%
    pivot_wider(names_from=c(measure),values_from=value) %>%
    as.data.frame()

  return(srl_time_and_distance)
}
...
calculateScenarioSRL <- function(srl_proportions,srl_trips,srl_time_and_distance) {
  workTrips <- srl_trips %>%
  ...
  workTripsProportions <- workTrips %>%
    ...
    mutate(base_car=round(prop_base_car*sa2_count),
           scen_car=round(prop_scen_car*sa2_count),
           base_pt.walk=round(prop_base_pt.walk*sa2_count),
           scen_pt.walk=round(prop_scen_pt.walk*sa2_count),
           base_pt.drive=round(prop_base_pt.drive*sa2_count),
           scen_pt.drive=round(prop_scen_pt.drive*sa2_count)) %>%
    mutate(base_count_diff=sa2_count-(base_car+base_pt.walk+base_pt.drive),
           scen_count_diff=sa2_count-(scen_car+scen_pt.walk+scen_pt.drive)) %>%
    mutate(base_car=ifelse(rank_base_car==1,base_car+base_count_diff,base_car),
           scen_car=ifelse(rank_scen_car==1,scen_car+scen_count_diff,scen_car),
           base_pt.walk=ifelse(rank_base_pt.walk==1,
                               base_pt.walk+base_count_diff,base_pt.walk),
           scen_pt.walk=ifelse(rank_scen_pt.walk==1,
                               scen_pt.walk+scen_count_diff,scen_pt.walk),
           base_pt.drive=ifelse(rank_base_pt.drive==1,
                                base_pt.drive+base_count_diff,base_pt.drive),
           scen_pt.drive=ifelse(rank_scen_pt.drive==1,
                                scen_pt.drive+scen_count_diff,scen_pt.drive)) %>%
    dplyr::select(orig_sa2,sa2_count,base_car,base_pt.walk,base_pt.drive,scen_car,
                  scen_pt.walk,scen_pt.drive) %>%
  ...
  workTripsMode <- mapply(
    chooseTrips,workTripsProportions$orig_sa2,workTripsProportions$sa2_count,
    workTripsProportions$base_car,workTripsProportions$base_pt.walk,
    workTripsProportions$base_pt.drive,workTripsProportions$scen_car,
    workTripsProportions$scen_pt.walk,workTripsProportions$scen_pt.drive,
  ...
  workTripsFinal <- workTrips %>%
    ...
    inner_join(srl_time_and_distance%>%filter(scenario=="base")%>%dplyr::select(-scenario),
               by=c("orig_sa2"="sa2","trip_mode_base"="mode")) %>%
    rename(trip_duration_base=time,trip_distance_base=distance) %>%
    inner_join(srl_time_and_distance%>%filter(scenario=="scen")%>%dplyr::select(-scenario),
...
srl_proportions <- calculateProportionsSRL(
  srl_df_location=srl_df_location
)

srl_time_and_distance <- calculateTimeDistanceSRL(
  srl_df_location=srl_df_location
)
...
trips_srl_scenario <- calculateScenarioSRL(
  srl_proportions=srl_proportions,
  srl_trips=srl_trips,
  srl_time_and_distance=srl_time_and_distance
)

trips_srl_scenario_final <- reformatScenarioSRL(
  trips_srl_scenario=trips_srl_scenario,
  speed_location=speed_location
)



write.csv(trips_srl_scenario_final, "Data/processed/trips_melbourne_scenarios_srl.csv", row.names=F, quote=T)



# sa2 <- st_read("./Data/vitm/sa2.sqlite")
# 
# test<- sa2 %>%
#   inner_join(srl_proportions,by=c("sa2_main16"="sa2"))
# st_write(test,"test_proportions.sqlite",delete_layer=T)
# 
# jpeg(file="car_trips.jpeg")
# hist(workTripsProportions$scen_car,
#      main="",
#      xlab="Change in car trips")
# dev.off()
# 
# jpeg(file="PT_walk_trips.jpeg")
# hist(workTripsProportions$scen_pt.walk,
#      main="",
#      xlab="Change in PT_walk trips")
# dev.off()
# 
# jpeg(file="PT_drive_trips.jpeg")
# hist(workTripsProportions$scen_pt.drive,
#      main="",
#      xlab="Change in PT_drive trips")
# dev.off()
# 
