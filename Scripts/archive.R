...
incidence_trends_breast_cancer_f <- incidence_trends_breast_cancer_f[index,] %>%
...
incidence_trends_uterine_cancer_f <- incidence_trends_uterine_cancer_f[index,] %>%
...
incidence_trends_lung_cancer_f <- incidence_trends_lung_cancer_f[index,] %>%
...
incidence_trends_lung_cancer_m <- incidence_trends_lung_cancer_m[index,] %>%
  mutate(year = from_age + sequence(incidence_trends_lung_cancer_m$agediff) - 1) %>%
  mutate(sex = "male") %>% 
  select(sex, tbalc, year)

#### COLON CANCERS FEMALES (bowel cancer, commonly referred as colorectal cancer)

incidence_trends_colon_cancer_f <- readxl::read_xls(incidence_trends_cancers, sheet = "Table S3b", range = "B6:L26") %>%
  mutate(incidence_trend_colon = log(`2020`/`2011`)/(2020-2011)) %>%
  extract(`Age group`, c("from_age", "to_age"), "(.+)–(.+)", remove=FALSE) %>% #Expand to one yr age groups
  mutate(from_age = as.numeric(case_when(`Age group`== "95+"  ~  "95",
                                         TRUE  ~  from_age)),
         to_age = as.numeric(case_when(`Age group`=="95+"  ~  "99",
                                       TRUE  ~  to_age)),
         agediff = to_age - from_age + 1,  # this will equal 5 
         carc = incidence_trend_colon) %>%
  rename(agegroup = `Age group`)

## Now stretch the data out using an index, to create a data frame with 1 row per year of age and create a variable for year of age. 
index <- rep(1:nrow(incidence_trends_colon_cancer_f), incidence_trends_lung_cancer_f$agediff)
incidence_trends_colon_cancer_f <- incidence_trends_colon_cancer_f[index,] %>%
  mutate(year = from_age + sequence(incidence_trends_colon_cancer_f$agediff) - 1) %>%
  mutate(sex = "female") %>% 
  select(sex, carc, year)


#### COLON CANCR MALES (bowel cancer, commonly referred as colorectal cancer)

incidence_trends_colon_cancer_m <- readxl::read_xls(incidence_trends_cancers, sheet = "Table S3a", range = "B6:L26") %>%
  mutate(incidence_trend_colon = log(`2020`/`2011`)/(2020-2011)) %>%
  extract(`Age group`, c("from_age", "to_age"), "(.+)–(.+)", remove=FALSE) %>% #Expand to one yr age groups
  mutate(from_age = as.numeric(case_when(`Age group`== "95+"  ~  "95",
                                         TRUE  ~  from_age)),
...
## Now stretch the data out using an index, to create a data frame with 1 row per year of age and create a variable for year of age. 
index <- rep(1:nrow(incidence_trends_colon_cancer_m), incidence_trends_colon_cancer_m$agediff)
incidence_trends_colon_cancer_m <- incidence_trends_colon_cancer_m[index,] %>%
  mutate(year = from_age + sequence(incidence_trends_colon_cancer_m$agediff) - 1) %>%
  mutate(sex = "male") %>% 
  select(sex, carc, year)
