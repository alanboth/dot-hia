...
mydata<-read.csv("cvd_deaths.csv",  header = TRUE)
#str(mydata)
#attach(mydata)
...
Females<-lm(log(mydata$Females)~mydata$Year, data=mydata)
summary(Females)
...
mtext("Forecasts for CVD_Females", side = 3, line = 1, cex = 1.2, font=2)
...
xrange <- 2029    # sets the range for the plot for the x variable
yrange <- 420    # sets the maximum for the y variable - you can also use max(predictFem) - also good to check the raw data as well as the predicted and forecasts
...
mtext("Forecasts for CVD_Males", side = 3, line = 1, cex = 1.2, font=2)
...
# firts, get the rates by dividing through by 100,000
...
names(Forecasts)[2]<- "Female_CVD_F"
names(Forecasts)[3]<- "Male_CVD_F"
...
write.csv(Forecasts, "C:/scratch/Research/DoT_RMIT/BelenData/ForecastCVDdeaths.csv")
