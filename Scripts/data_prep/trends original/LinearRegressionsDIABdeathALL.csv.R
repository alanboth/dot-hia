...
mydata<-read.csv("diabetes_death_all.csv",  header = TRUE)
#str(mydata)
#attach(mydata)
my_data <- data
...
new_YEAR=data.frame(seq(1997,2028,1))   # creates a sequence for YEAR but include an additional 10 years
new_YEAR
...
plot(xrange, yrange,
     ...
     ylab ="Rate_Diab_deaths_all_Females",
     xlab ="Year", 
     pch=19,
     cex=1.3)
#xaxt = "YEAR")
...
mtext("Forecasts for Diab_deaths_all_Females", side = 3, line = 1, cex = 1.2, font=2)
...
plot(xrange, yrange,
     ...
     ylim=c(60, 80),
     type = "n",       # this suppressess the plot points (you are are using the x, y to create the correct plot size)
     ylab ="Rate_Diab_deaths_all_Males",
     xlab ="Year", 
     pch=19,
     cex=1.3)
#xaxt = "YEAR")
...
mtext("Forecasts for Diab_deaths_all_Males", side = 3, line = 1, cex = 1.2, font=2)
...
# adjust the ERP file to suit the years 1997 - 2028
...
names(Forecasts)[2]<- "Female_DiabAll_F"
names(Forecasts)[3]<- "Male_DiabAll_F"
...
write.csv(Forecasts, "C:/scratch/Research/DoT_RMIT/BelenData/ForecastDiabDeathAll.csv")
