...
mydata<-read.csv("cvd_hospital.csv",  header = TRUE)
#str(mydata)
#attach(mydata)
...
new_YEAR=data.frame(seq(2001,2028,1))   # creates a sequence for YEAR but include an additional 10 years
new_YEAR
...
mtext("Forecasts for CVDHosp_Females", side = 3, line = 1, cex = 1.2, font=2)
...
mtext("Forecasts for CVDHosp_Males", side = 3, line = 1, cex = 1.2, font=2)
...
# adjust the ERP file to suit the years 2001 - 2028
...
names(Forecasts)[2]<- "Female_CVDHosp_F"
names(Forecasts)[3]<- "Male_CVDHosp_F"
...
write.csv(Forecasts, "C:/scratch/Research/DoT_RMIT/BelenData/ForecastCVDHosp.csv")
