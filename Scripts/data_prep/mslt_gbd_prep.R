...
calculateMSLT <- function(population_melbourne_location,deaths_melbourne_location,gbd_wider_location) {
  ...
  pop_melb <- read.csv(population_melbourne_location, as.is=T,
...
#####ALAN comments below
#### We will add disbayes outcomes and then this feeds onto mslt code, for now, we use placeholder

### the code below is old, but I will need potentially, so please leave there for now
...
#   
#   data_1 <-  filter(gbd_data, area == a)
#   data_2 <- filter(disbayes_output, area == a)
#   
