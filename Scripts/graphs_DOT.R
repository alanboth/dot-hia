...
GraphsMode <- function(age_val,sex_val,scen_val) {
  ...
  dataFiltered <- output_transport_modes %>% 
    ...
    mutate(scenario=factor(scenario, levels=c("bl","sc"), labels=c("Baseline", scen_label))) %>%
    mutate(mode=factor(mode,
                       levels=c("walking","bicycle","public.transport","pt.walk","pt.drive","car","other"),
                       labels=c("Walking","Cycling","Non-work public\ntransport","Walking to public\ntransport",
                                "Driving to public\ntransport","Driving","Other")))
